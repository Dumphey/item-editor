...
public class ItemEditorCommand implements CommandExecutor, TabCompleter {
    ...
    private final at.dumphey.itemeditor.itemeditor.ItemEditor itemEditor;

    public ItemEditorCommand() {
        this.itemEditor = at.dumphey.itemeditor.itemeditor.ItemEditor.INSTANCE;
    }
    ...
    private void handleGive(CommandSender sender, String[] args) {
        ...
        final SerializedItem item = at.dumphey.itemeditor.itemeditor.ItemEditor.INSTANCE.getItemStorage().getItem(itemName);
        if (item == null) {
        ...
        player.getInventory().addItem(at.dumphey.itemeditor.itemeditor.ItemEditor.INSTANCE.personalizeItem(toGive, player));
    ...
    private void handleDelete(CommandSender sender, String[] args) {
        ...
        if (at.dumphey.itemeditor.itemeditor.ItemEditor.INSTANCE.getItemStorage().deleteItem(args[1])) {
    ...
    private void handleList(CommandSender sender, String[] args) {
        if (args.length > 1 || !(sender instanceof Player)) {
            if (!(sender instanceof Player) || args[1].equalsIgnoreCase("plain")) {
                List<String> itemNames = at.dumphey.itemeditor.itemeditor.ItemEditor.INSTANCE.getItemStorage()
                        .getItems()
                        .stream()
        ...
        at.dumphey.itemeditor.itemeditor.ItemEditor.INSTANCE.getUiManager().open(((Player) sender), new ItemListUiScreen(((Player) sender)));
    }
    ...
    private void handleGet(Player player, String[] args) {
        ...
        String name = args[1];
        SerializedItem item = at.dumphey.itemeditor.itemeditor.ItemEditor.INSTANCE.getItemStorage().getItem(name);
        ...
        player.getInventory().setItemInMainHand(at.dumphey.itemeditor.itemeditor.ItemEditor.INSTANCE.personalizeItem(item.getItemStack(), player));
    ...
    private void handleSave(Player player, String[] args) {
        ...
        try {
            at.dumphey.itemeditor.itemeditor.ItemEditor.INSTANCE.getItemStorage().saveItem(name, player.getInventory().getItemInMainHand());
            player.playSound(player.getLocation(), Sound.ENTITY_EXPERIENCE_ORB_PICKUP, 1, 1);
            Messages.send(player, "§aYour §o" + getItemName(player.getInventory()
                    .getItemInMainHand()) + " §awas saved under the name §o" + name + "§a.");
        } catch (IOException e) {
    ...
    private String getItemName(ItemStack itemStack) {
        ...
        return NameUtils.enumToFriendlyName(itemStack
                .getType()
                .name());
    }
    ...
    private List<String> getItemNames() {
        return at.dumphey.itemeditor.itemeditor.ItemEditor.INSTANCE.getItemStorage()
                .getItems()
                .stream()
                .map(SerializedItem::getName)
    ...
    private void printHelp(CommandSender sender) {
        sender.sendMessage("");
        sender.sendMessage(
                " §6§lI§6tem§e§lE§editor §7v" + ItemEditor.getInstance().getDescription().getVersion());
        sender.sendMessage("");
